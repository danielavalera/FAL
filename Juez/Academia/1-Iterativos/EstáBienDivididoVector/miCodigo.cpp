 ...
 * 
 * 
 * REVISAR, PORQUE COMO LO HE HECHO DIFERENTE ALOMEJOR CAMBIA
 */
...
bool resolver(vector<int>& v, int p) {
    ...
    for (int i = 0; i <= p; i++)
        if (maxIzq < v[i]) maxIzq = v[i];
    

    for (int i = p + 1; i < v.size(); i++)
        if (maxIzq < v[i]) bienDividido = true;
        else bienDividido = false;
...
void resuelveCaso() {
    int tamV, p; cin >> tamV >> p;
    vector<int> v(tamV);

    for (int i = 0; i < tamV; i++)
        cin >> v[i];

    if (p == tamV - 1 || resolver(v, p)) cout << "SI" << '\n';
    else cout << "NO" << '\n';
}

int main() {
#ifndef DOMJUDGE
    std::ifstream in("BienDivididoVector.txt");
