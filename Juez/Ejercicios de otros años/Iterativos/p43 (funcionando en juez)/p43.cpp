...
int resolver(std::vector<int> const& datos) {
	int posMaximo = datos[0];
	int sumaSinMaximo = datos[0];
	int contadorMaximo = 1;
	for (int i = 1; i < datos.size(); i++)
	{
		if (datos[i] > posMaximo) {
			posMaximo = datos[i];
			contadorMaximo = 1;
		}
		else if (datos[i] == posMaximo) {
			++contadorMaximo;
		}
		sumaSinMaximo += datos[i];
	}
	sumaSinMaximo = sumaSinMaximo - (posMaximo*contadorMaximo);
